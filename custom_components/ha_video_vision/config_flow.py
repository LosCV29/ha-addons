"""Config flow for HA Video Vision - Auto-Discovery with DeepFace."""
...
class VideoVisionConfigFlow(config_entries.ConfigFlow, domain=DOMAIN):
    ...
    async def async_step_select_model(
        ...
        if user_input is not None:
            ...
            return await self.async_step_deepface()
    ...
    async def async_step_deepface(
        self, user_input: dict[str, Any] | None = None
    ) -> FlowResult:
        """Configure DeepFace facial recognition."""
        errors = {}

        if user_input is not None:
            self._data[CONF_DEEPFACE_ENABLED] = user_input.get(CONF_DEEPFACE_ENABLED, False)
            self._data[CONF_DEEPFACE_URL] = user_input.get(CONF_DEEPFACE_URL, DEFAULT_DEEPFACE_URL)
            self._data[CONF_DEEPFACE_TOLERANCE] = user_input.get(CONF_DEEPFACE_TOLERANCE, DEFAULT_DEEPFACE_TOLERANCE)

            # Validate DeepFace connection if enabled
            if self._data[CONF_DEEPFACE_ENABLED]:
                is_valid = await self._test_deepface(self._data[CONF_DEEPFACE_URL])
                if not is_valid:
                    errors["base"] = "deepface_connection_failed"
                else:
        ...
        return self.async_show_form(
            step_id="deepface",
            data_schema=vol.Schema({
                vol.Optional(CONF_DEEPFACE_ENABLED, default=DEFAULT_DEEPFACE_ENABLED): bool,
                vol.Optional(CONF_DEEPFACE_URL, default=DEFAULT_DEEPFACE_URL): str,
                vol.Optional(CONF_DEEPFACE_TOLERANCE, default=DEFAULT_DEEPFACE_TOLERANCE): vol.All(
    ...
    async def _test_deepface(self, url: str) -> bool:
        """Test connection to DeepFace server."""
        try:
            async with aiohttp.ClientSession() as session:
                test_url = f"{url.rstrip('/')}/status"
            ...
            _LOGGER.warning("DeepFace connection test failed: %s", e)
            return False
...
class VideoVisionOptionsFlow(config_entries.OptionsFlow):
    ...
    async def async_step_init(
        ...
        return self.async_show_menu(
            ...
            menu_options={
                ...
                "deepface": "Configure Facial Recognition",
    ...
    async def async_step_deepface(
        ...
        if user_input is not None:
            deepface_enabled = user_input.get(CONF_DEEPFACE_ENABLED, False)
            deepface_url = user_input.get(CONF_DEEPFACE_URL, DEFAULT_DEEPFACE_URL)
            deepface_tolerance = user_input.get(CONF_DEEPFACE_TOLERANCE, DEFAULT_DEEPFACE_TOLERANCE)

            if deepface_enabled:
                is_valid = await self._test_deepface(deepface_url)
                if not is_valid:
                    errors["base"] = "deepface_connection_failed"
            ...
            if not errors:
                new_options = {
                    **self._entry.options,
                    CONF_DEEPFACE_ENABLED: deepface_enabled,
                    CONF_DEEPFACE_URL: deepface_url,
                    CONF_DEEPFACE_TOLERANCE: deepface_tolerance,
                }
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                vol.Optional(
                    CONF_DEEPFACE_ENABLED,
                    default=current.get(CONF_DEEPFACE_ENABLED, DEFAULT_DEEPFACE_ENABLED)
                ): bool,
                vol.Optional(
                    CONF_DEEPFACE_URL,
                    default=current.get(CONF_DEEPFACE_URL, DEFAULT_DEEPFACE_URL)
                ): str,
                vol.Optional(
                    CONF_DEEPFACE_TOLERANCE,
                    default=current.get(CONF_DEEPFACE_TOLERANCE, DEFAULT_DEEPFACE_TOLERANCE)
                ): vol.All(vol.Coerce(float), vol.Range(min=0.1, max=1.0)),
            }),
            errors=errors,
            description_placeholders={
                "deepface_hint": "Enter the URL of your DeepFace facial recognition server (e.g., http://homeassistant.local:8100)",
            },
        )
    ...
    async def async_step_voice_aliases(
        ...
        camera_list = "\n".join([f"  - {c}" for c in selected_cameras]) or "  (none selected)"
        ...
        return self.async_show_form(
            ...
            description_placeholders={
                ...
                "example": "front door:camera.front_porch\nbackyard:camera.back_yard",
            },
        )
    ...
    async def async_step_video_quality(
        ...
        if user_input is not None:
            new_options = {
                **self._entry.options,
                CONF_VIDEO_DURATION: user_input.get(CONF_VIDEO_DURATION, DEFAULT_VIDEO_DURATION),
                CONF_VIDEO_WIDTH: user_input.get(CONF_VIDEO_WIDTH, DEFAULT_VIDEO_WIDTH),
                CONF_SNAPSHOT_DIR: user_input.get(CONF_SNAPSHOT_DIR, DEFAULT_SNAPSHOT_DIR),
                CONF_SNAPSHOT_QUALITY: user_input.get(CONF_SNAPSHOT_QUALITY, DEFAULT_SNAPSHOT_QUALITY),
            }
            return self.async_create_entry(title="", data=new_options)
        ...
        return self.async_show_form(
            ...
            data_schema=vol.Schema({
                ...
                vol.Optional(
                    ...
                    selector.SelectSelectorConfig(
                        options=[
                            {"value": "320", "label": "320p (Low)"},
                            {"value": "480", "label": "480p (Medium)"},
                            {"value": "640", "label": "640p (Default)"},
                            {"value": "720", "label": "720p (High)"},
    ...
    async def async_step_ai_settings(
        ...
        if user_input is not None:
            new_options = {
                **self._entry.options,
                CONF_VLLM_MAX_TOKENS: user_input.get(CONF_VLLM_MAX_TOKENS, DEFAULT_VLLM_MAX_TOKENS),
                CONF_VLLM_TEMPERATURE: user_input.get(CONF_VLLM_TEMPERATURE, DEFAULT_VLLM_TEMPERATURE),
            }
            return self.async_create_entry(title="", data=new_options)
